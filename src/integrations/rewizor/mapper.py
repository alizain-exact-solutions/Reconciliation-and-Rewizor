...
from src.integrations.rewizor.constants import (
...
from src.integrations.rewizor.schemas import EPPDocument, EPPHeader, EPPVatRow
...
def _is_foreign_supplier(invoice: Dict[str, Any]) -> bool:
    ...
    # Polish NIP is exactly 10 digits; empty NIP with a named vendor → likely foreign
...
def map_invoice_to_epp(
    ...
    *,
    ...
    # Ensure exchange_rate is never 0
    ...
    header = EPPHeader(
        ...
        field_35=gross,  # duplicate gross (field 35 in reference)
        currency=currency,
    ...
    # ── VAT breakdown ────────────────────────────────────────────────────
    if is_non_vat:
        # Non-VAT documents (WB, RK, PK, DE): single zero-rate row
    ...
    if vat_breakdown:
        ...
        for row in vat_breakdown:
            ...
            # Correct reverse-charge rows: symbol "00" must have rate -5.00
