...
from src.integrations.rewizor.constants import DOC_TYPE_PURCHASE_INVOICE, VALID_DOC_TYPES
from src.services.reporting.rewizor_service import export_from_db, process_and_export
...
async def rewizor_upload_async(
    ...
    from src.workers.tasks.report_task import rewizor_export_task
...
async def rewizor_db_export(
    invoice_ids: Optional[List[int]] = Query(None, description="Specific invoice IDs"),
    status: str = Query("PENDING", description="Invoice status filter"),
    doc_type: str = Query(DOC_TYPE_PURCHASE_INVOICE, description="Document type"),
):
    """Generate an EPP file from invoices already stored in the database.

    Either provide explicit ``invoice_ids`` or filter by ``status``.
    """
